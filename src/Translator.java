...
public class Translator {
	 ...
	 *
	 * @param dirPath
	 *            the path to the directory with all necessary files
	 * @throws Exception
	 *             the exception
	 */
	...
	private void createMainFile(String inputFile) throws IOException {
		...
		while ((line = reader.readLine()) != null) {
			...
			if (sectionBegins(line)) {
				 ...
				 * be in body of file
				 */
				if ((line.contains("Figure Legends")
	...
	private void clearMainFile() {
		InputStream is;
		try {
			is = new FileInputStream(MainWindow.mainPath
					+ "/LaTEXtoXML/mainFileBad.tex");
			...
			String line = "";
			while ((line = reader.readLine()) != null) {
				line = line.replaceAll("\\u005c\\u0071\\u0075\\u0061\\u0064\\u0023\\u0031\\u002e", "");
				os.println(line);
			}
			is.close();
			os.close();
	 ...
	 * 
	 * @param line
	 *            Checks if the line is a comment or an empty
	 * @return
	 */
	 ...
	 *
	 * @param fileName
	 *            the bibliography file name
	 * @param os
	 *            the stream in which bib is written
	 * @throws IOException
